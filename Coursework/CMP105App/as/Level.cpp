...
void Level::drawTile(sf::Vector2i pos,sf::Color color) {
...
void Level::generateTileMap(const std::vector<Node>& closedNodes, const std::vector<Node>& openNodes, const std::vector<sf::Vector2i>& path)
{
	int tilesize = 64;
...
int Level::calculateHueristic(int startX, int startY, int endX, int endY)
{
...
void Level::scanMap(Map map) {//this Function is sponsered by A*
	if (!calledOnce) {
		calledOnce = 1;
